...
def process_school_bus_sheet(content, sheet_name, override_enabled, override_lob, override_segment, override_policy_type):
    ...
    try:
        ...
        # Appended pattern: Search for "seating capacity" in any column
        seating_row = None
        seating_col = -1
        remarks = ""
        for i in range(table_start_row + 1, min(table_start_row + 5, len(df))):
        ...
        if seating_col > 0:  # New pattern with seating as column
            state_col = 0
            rto_col = 1
            payin_start = seating_col + 1
            # Extract contract headers
            contract_types = []
            for k in range(payin_start, len(df.columns)):
                header = str(df.iloc[seating_row, k]).strip()
                if header:
                    contract_types.append(header)
            contracts = [(payin_start + idx, contract_type) for idx, contract_type in enumerate(contract_types)]
        else:  # Old pattern
            state_col = 0
            rto_col = 1
            contracts = [(2, "In name of School"), (3, "On Contract (Transporter)"), (4, "On Contract (Individual)"), (5, "Contract transporter")]
        ...
        for row_idx in range(data_start_row, len(df)):
            ...
            for col_idx, contract_type in contracts:
                payin_val = df.iloc[row_idx, col_idx]
                payin = safe_float(payin_val)
                if payin is None:
                    continue
                ...
                if seating_col > 0:
                    seating = str(df.iloc[row_idx, seating_col]).strip() if pd.notna(df.iloc[row_idx, seating_col]) else ""
                    if seating:
                        record["Original Segment"] = f"School Bus - {contract_type} ({seating})"
                if remarks:
                    record["Remarks"] = remarks
